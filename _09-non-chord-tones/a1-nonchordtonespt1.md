...
1. First, some non-chord tones can occur within a single harmony *or* across two harmonies, meaning that the preparation, NCT, and resolution can either be entirely within one chord *or* spread across two different harmonies. Some NCTs can *only* occur across two chords. For example, a passing tone can occur either within a single chord or span two chords, but suspensions and retardations can *only* happen across two chords; it is not possible to have suspension or retardation within a single chord. Any NCT with restrictions such as this are noted in their descriptions, and it is imperative that you learn the context for each of the non-chord tones, so that you can quickly identify these issues. There are no NCTs that can *only* occur within a single harmony.
...
3. As you learn each of these non-chord tones, you will begin to notice these patterns in music, even when no NCT is present. If you would like to use a motion description for a chord tone, such as passing or neighbor, you can replace the word *tone* with the word *figure*. So a "passing figure" is a pitch that is approached by step and left by step in the same direction, but all three of the pitches that make up the preparation, passing tone, and resolution are chord tones.
